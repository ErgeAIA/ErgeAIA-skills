...
## 2026-09-18 命名偏好 SSOT 下沉 references（渐进式披露）

| # | 既有决策（旧值） | 处置 | 新值 / 原因 |
|---|------------------|------|------------|
| 1 | 「唯一全文见 `Skills-Depot/AGENTS.md`『技能命名偏好（两库 SSOT）』」 | 改 | 全文迁至 `Skills-Depot/references/skill-naming-pref.md`；AGENTS 只留指针（渐进式披露） |

## 2026-09-18 命名偏好上收统一层 SSOT

| # | 既有决策（旧值） | 处置 | 新值 / 原因 |
|---|------------------|------|------------|
| 1 | 「命名: Skill 名 hyphen-case，长度 `<=64`」 | 改 | 机器约束保留（hyphen-case / ≤64 / name=目录）；语义分层与命名公式（内容链 1–2 音节拼音+功能即名+无后缀+门面仿古定义；工具/编排英文连字符；4 音节全拼仅工作流）唯一全文上收 `Skills-Depot/AGENTS.md`「技能命名偏好（两库 SSOT）」，本仓只留指针 |
...
## 2026-09-13 自维护协议补充「推送前版本对齐检查」

用户要求：版本漂移多次发生，在推送前强制检查一次。
