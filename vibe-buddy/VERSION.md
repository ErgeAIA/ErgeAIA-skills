...
**vibe-distill / vibe-init 真跑修复（xiaobu 实跑）**

用户指定废弃项目 `xiaobu`（React 19 + Vite 8 + Tailwind 4；1 个提交 + 1068 行未提交改动）作 D2 真跑对象。完整跑完 `vibe-init` → `vibe-distill`，暴露并修复 5 处：
...
真跑产出（xiaobu 侧）：`AGENTS.md` 77 行六节契约、`docs/.ai/` 五份过程文档 + `experience/`（首次蒸馏出 5 条经验：ui 域 T1/T2/G1，frontend 域 R1/T3）+ `docs/handoff/`，`codegraph init` 建索引 21 文件 / 242 节点。
...
真跑 lingjian 的验收核对当场暴露一处空洞：§7 自检门（五项 + 另核对四条）里**没有任何一条检查「`Permissions` 的义务集合是否与 §5 一致」**——只查句式四选一、黑名单、写入闸、行数、标题语言、`mode` 单值、`References` / `Permissions` 路径存在、§4b 已落地未覆盖、changelog 有对应行。后果：**一份缺了接管义务的 `AGENTS.md` 能通过全部自检门**——这正是上一轮 F1 未被发现的原因，真跑时靠的是**手工比对**才补上。
