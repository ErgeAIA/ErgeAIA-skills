...
| 触发词 | 说什么 | 做什么 |
|---|---|---|
| `vibe-init` | 初始化项目 / 生成 AGENTS.md | 建立 `AGENTS.md` 与 `docs/.ai/` 三件套、`docs/handoff/`；半程项目只补齐缺失文档，不重写已有文件 |
...
```
项目根/
├── AGENTS.md                        # 规则契约（含过程文档指引与收尾同步规则）
├── CLAUDE.md                        # 可选，指向 AGENTS.md 的指针
└── docs/
    ├── .ai/
    │   ├── project-progress.md      # 项目进度，实时更新；任务开始时先读它
...
| 收尾同步写进契约 | 不写进 `AGENTS.md` 就没人执行。每完成一次任务要更新哪些文档，必须让 AI 每次会话都读到 |
