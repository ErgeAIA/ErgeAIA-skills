...
| 判定 | 动作 |
| ---- | ---- |
| 返回假或报错 | 执行 `git init`，报告写「已初始化仓库，默认分支 <分支名>」；分支名读 `<project>/.git/HEAD` 的 `ref: refs/heads/<名称>`——**`git init` 的输出不含分支名，不得凭经验填 `main` / `master`** |
...
| `git log` 返回 128 且报 `does not have any commits yet` | **不是失败**：该仓库尚无提交，判「无提交」后继续 |
