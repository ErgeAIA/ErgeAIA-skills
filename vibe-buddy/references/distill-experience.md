...
**首次蒸馏**时 `changelog.md` 还不存在（它正是由首次蒸馏创建）：跳过比对，建台账并写第一条；此后每次蒸馏前都必须比对。
...
| 情形 | 写法 |
|---|---|
| 改动已提交 | `<首个commit>..<末个commit>` |
| 改动尚未提交 | `<最后一个commit>..工作区（未提交）` |
| 首次蒸馏（项目存量无轮次边界） | `<当前HEAD>..工作区`，并在主题里注明「首次蒸馏：存量盘点」 |

**「原料」要写成语义来源清单**（读过哪些文件、命令、来源区间），不能只写 git 区间——git 区间只回答「代码变没变」，回答不了「这个来源我看过没」。

**复蒸判定**（第二次及以后）：

| 情形 | 处置 |
|---|---|
| 项目自身改动区间完全重叠，且上一轮「有意跳过」已穷举 | **拒绝**，指向那一轮，不写任何文件 |
| 区间重叠，但发现上一轮**既未列产出、也未列跳过**的来源 | 属「部分重叠」：只蒸该来源的新增部分，并在备注写明上一轮为何漏掉 |
