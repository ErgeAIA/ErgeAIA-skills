...
| 序 | 事实源 | 取什么 |
| -- | ------ | ------ |
| 1 | `git log --oneline -n <N>`、`git status --short`、`git diff --stat` | 已有哪些提交、工作区改了什么、改动规模；**仅本项目在 Git 仓库内时可用** |
...
- git 输出只用于核对与重建事实，**不搬进文档**；需要引用时写 commit hash
- 项目不是 Git 仓库 → 跳过第 1 项，视为无此事实源，不报错
- 第 1 项显示的工作区改动若落在 `docs/.ai/` 之外，**不在本技能职责内**，只作核对参照，不改也不记
