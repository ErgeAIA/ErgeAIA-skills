...
- 半程合成 → 先执行 §3 再生成
- 已初始化优化 → 增量维护，禁止整体重写
- 模式之上先过**初始化状态机 A–E**（见 `references/init-agents-md.md`）：项目根存在 `CLAUDE.md` 的状态 C / D 走 §2b 受控迁移；**任何状态都不创建 `CLAUDE.md`**
...
## 2b 协作契约迁移（CLAUDE.md → AGENTS.md）

`AGENTS.md` 是唯一项目级协作契约。迁移中 `CLAUDE.md` 只是**输入源**，`AGENTS.md` 是唯一**输出源**；迁移完成后 `CLAUDE.md` 不得继续作为项目级协作契约存在，也不得降级为长期维护的指针。

**受控重组例外**：迁移允许把目标契约按 §5 模板重组（不受「增量维护禁止整体重写」约束），前提是每条处置都在层 C 留痕、先写目标、验证目标、最后才删源——普通维护仍是一行不动地增量，迁移才可重组。

**保全规则**——旧 `CLAUDE.md` 中每条有效规则必须落入三处之一，不得出现「既不进 `AGENTS.md` 也无 drop 记录」的悬空规则：

| 处置 | 去处 |
| ---- | ---- |
| keep / merge | 写入 `AGENTS.md`（merge 须与既有规则去重合并） |
| update | 以事实证据改写后写入 `AGENTS.md`，层 C 记「旧值 → update → 新值」 |
| drop | 不进 `AGENTS.md`，层 C 记「旧值 → drop → 原因」；Claude 专属语法、Claude 专属工具说明、已过时规则属此类 |

**冲突裁决阶梯**（状态 D：两份契约并存）——依据是事实，不是文件名，不得规定「AGENTS 永远覆盖 CLAUDE」：
...
**写入与删除次序**：`AGENTS.md` 落盘并实读验证通过、层 C 迁移记录写入完成之后，才允许删除 `CLAUDE.md`；任何一步失败即保留源文件，终止回复写明「迁移未完成，原 CLAUDE.md 保留」。执行细则与安全闸见 `references/init-agents-md.md`。
