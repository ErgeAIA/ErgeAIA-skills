...
## [2.5.0] - 2026-09-26 · 创建路径补 Grill 闸：意图不清先对齐需求

> 起因：CREATE 是唯一没有需求发现环节的路径——模板 + `init` + `validate` 只保证「格式对」，不保证「建的是该建的」。AI 倾向快速交付，会把未澄清的需求直接固化进技能，之后靠补丁维持，维护成本远高于当场问清。

### 新增

- `references/creation.md` 新增「创建前 Grill 闸（条件触发，不强制）」：四个进入条件（需求含糊 / 空白技能 / 边界不清 / 一次建多个）、两个跳过条件、**内化的决策树方法**（前沿 frontier、逐轮问、每题附推荐、事实自己查而决定交用户、前沿空才动手）、六个覆盖维度（按复杂度取舍、不设配额）、产物为可选的「创建决策」。
- 「创建时只定义三件事」处补指针：三件事从 Grill 来，意图不清别直接套模板。

### 设计取舍

- **自持，不依赖外部技能**：grill 思想（决策树 / 前沿 / 轮次 / 事实与决定分工 / 终止条件）**内化进本技能**，不指向外部 `grilling` 技能——技能须自包含（本仓 2026-09-26 已立「技能必须自包含」原则），且避免两套表述日后漂移。
- **不设配额**：只给维度清单，不写「至少 N 问 / ≥N 个场景」——写死数字会诱导凑数（同 `optimization.md` §11 口径）。
- **条件触发**：意图清晰照旧直接创建；Grill 是需求对齐手段，不是仪式。

### 变更

- `SKILL.md` 构建路径步骤 2 补 Grill 前置与理由；资源指引表 `creation.md` 行同步。
- 评审 / 优化 / 校验三条路径与 CLI 未变。
