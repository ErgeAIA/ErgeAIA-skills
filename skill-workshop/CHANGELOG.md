...
> 起因：CREATE 是三条路径里唯一没有需求发现环节的——读模板 + `init` + `validate` 只保证「格式对」，不保证「建的是该建的」。AI 倾向快速交付，会把未澄清的需求直接固化进技能，之后靠补丁维持，维护成本远高于当场问清。

### 新增

- `references/creation.md` 新增「创建前 Grill 闸（条件触发，不强制）」：四个进入条件（需求含糊 / 空白技能 / 边界不清 / 一次建多个）、两个跳过条件（职责与 IO 已可直接写出 / 用户说照这个写）、六问清单（Core Task / 触发 / 输入输出 / 边界 / 失败模式 / 约束）、frontier 逐轮问法与「事实自己查、决策交用户」、产物为 5–10 行创建决策。
- 「创建时只定义三件事」处补指针：三件事从 Grill 来，意图不清别直接套模板。

### 变更

- `SKILL.md` 路由表 CREATE 行：意图不清 → 先过 Grill 闸；清晰 → 直接模板 + `init`。产出列补「+ 创建决策」。
- 构建路径步骤 2 补 Grill 前置与理由；资源指引表 `creation.md` 行同步。

### 未动

- 不强制：意图清晰照旧直接创建，Grill 是需求对齐手段、不是仪式。
