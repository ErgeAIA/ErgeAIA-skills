...
- 先确认目标 Skill 已通过 `python scripts/skill_cli.py validate <skill-dir>`
- 若目标 Skill 提供 `scripts/validate_skill.py`，先补跑工程硬校验
- 若 `eval` 启动即失败，先检查 `eval-set` 是否满足 `query` 唯一且 `should_trigger` 为布尔值
