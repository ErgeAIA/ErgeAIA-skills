...
- 标题下的版本信息块（`> **版本**: vX.Y.Z`）——可选；存在外部变更记录（CHANGELOG.md / VERSION.md）时，头部版本块与文末版本历史 section 可省略
- 文末 `## 版本历史`——可选；当使用外部变更记录（CHANGELOG.md / VERSION.md）承载版本演进时，可省略内联版本历史
...
> 版本演进的**权威外部记录**是 `CHANGELOG.md`（Keep a Changelog 规范，由 changelog-manager 维护）或 `VERSION.md`。当技能使用外部变更记录时，SKILL.md 的头部版本块与文末 `## 版本历史` 均可省略，校验器以外部记录为 fallback，不强制内联历史。
...
**条目数量约束**：内联版本历史保留最近 5 条，超出时裁剪最早的条目。外部 `CHANGELOG.md` 保留完整历史，不受 5 条上限约束。
