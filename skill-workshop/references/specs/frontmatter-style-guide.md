...
## 九、description 字段联锁规则（W7 必读）

`description` 必须同时满足以下 3 个文件的联锁约束，否则 W7 description 审计会标 T1-T5 之一：
...
**违反**：YAML 块（应单行）+ 无 Pushy 风格 + 无"Not for"边界 + 字符数 110 但丢了 Pushy 句式。
...
**满足**：单行 string + Pushy "Use this skill whenever" + "Invoke on" 触发词列举 + "Not for" 边界声明 + 251 字符（在 200-400 区间）。
