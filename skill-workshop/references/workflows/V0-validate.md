...
- 版本块（头部版本块 / 文末版本历史）**本身可选**：文档完全无版本块时跳过三处比对；但**无版本块且无版本变更记录（CHANGELOG.md / VERSION.md）时判 FAIL**（`Missing version info in document header`，对齐 quick_validate.py 实际行为）；存在版本变更记录（CHANGELOG.md / VERSION.md）时以其为版本真源、跳过头部/文末比对
...
- **信息提示（未脚本化）**：frontmatter 内是否塞入了应归版本变更记录（CHANGELOG.md / VERSION.md）的元数据（如 `created` / `updated` / `origin`）
