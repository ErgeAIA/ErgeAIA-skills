"""Generate an HTML report from `python scripts/skill_cli.py loop` output."""
...
""")
        ...
        for qinfo in test_queries:
            ...
            html_parts.append(
                f'                <td class="result test-result {css_class}">{icon}<span'
                f' class="rate">{triggers}/{runs}</span></td>\n'
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "input", help="Path to JSON output from `python scripts/skill_cli.py loop` (or - for stdin)"
    )
