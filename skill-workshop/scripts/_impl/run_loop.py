...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Save all outputs (results.json, report.html, log.txt) to a timestamped subdirectory"
            " here"
        ),
    )
    args = parser.parse_args(argv)

    eval_set, message = load_and_validate_eval_set(Path(args.eval_set))
    if eval_set is None:
    ...
    if args.report != "none":
        ...
        live_report_path.write_text(
            "<html><body><h1>Starting optimization loop...</h1><meta http-equiv='refresh'"
            " content='5'></body></html>",
            encoding="utf-8",
        )
