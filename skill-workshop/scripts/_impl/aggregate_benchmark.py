            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                for exp in raw_expectations:
                    if "text" not in exp or "passed" not in exp:
                        print(
                            f"Warning: expectation in {grading_file} missing required fields (text,"
                            f" passed, evidence): {exp}"
                        )
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines = [
        ...
        (
            "**Evals**:"
            f" {', '.join(map(str, metadata['evals_run']))} ({metadata['runs_per_configuration']} runs"
